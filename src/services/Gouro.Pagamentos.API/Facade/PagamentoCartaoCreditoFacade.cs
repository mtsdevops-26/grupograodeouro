...
using Gouro.Pagamentos.GouroPag;

namespace Gouro.Pagamentos.CardAntiCorruption
{
    public class PagamentoCartaoCreditoFacade : IPagamentoFacade
    {
        ...
        {
            var gouroPagSvc = new GouroPagService(_pagamentoConfig.DefaultApiKey, _pagamentoConfig.DefaultEncryptionKey);

            var cardHashGen = new CardHash(gouroPagSvc)
            {
            ...
            var transacao = new Transaction(gouroPagSvc)
            {
                CardHash = cardHash,
                CardNumber = pagamento.CartaoCredito.NumeroCartao,
                CardHolderName = pagamento.CartaoCredito.NomeCartao,
        ...
        public async Task<Transacao> CapturarPagamento(Transacao transacao)
        {
            ...
            var transaction = ParaTransaction(transacao, gouroPagSvc);
        ...
        public static Transaction ParaTransaction(Transacao transacao, GouroPagService gouroPagService)
        {
            return new Transaction(gouroPagService)
            {
